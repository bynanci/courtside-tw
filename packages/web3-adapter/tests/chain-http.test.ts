...
const request: RegistryRequest = {
  ...
  cidDigest: DIGEST,
  snapshotId: "0190f7b0-7c4b-7e3a-8f12-123456789abc",
  schemaVersion: "1",
  publishedAt: "2026-09-12T00:00:00Z",
  idempotencyKey: "snapshot:0190f7b0-7c4b-7e3a-8f12-123456789abc:v1"
}
...
async function fixture(t: TestContext) {
  ...
  const state = {
    ...
    failRpc: false,
    failBroker: false,
    preflightDigest: `0x${"00".repeat(32)}`,
    confirmedDigest: `0x${"ab".repeat(32)}`,
    transactionOverrides: {} as Record<string, unknown>,
    receiptOverrides: {} as Record<string, unknown>
  }
  const server = createServer(async (incoming, outgoing) => {
    ...
    if (incoming.url === "/signer") {
      ...
      if (durable.has(key) && durable.get(key) !== fingerprint) {
        outgoing.writeHead(409)
        outgoing.end("{}")
        return
      }
      if (!durable.has(key)) {
        durable.set(key, fingerprint)
        actualTransactions += 1
      }
    ...
    switch (method) {
      case "eth_chainId":
        ...
        break
      case "eth_call":
        result = params[1] === "latest" ? state.preflightDigest : state.confirmedDigest
        break
      ...
      case "eth_getBlockByNumber":
        result = { number: "0xa", hash: state.canonical }
        break
      default:
        outgoing.end(
          JSON.stringify({
  ...
  const options = {
    ...
    signer: createManagedSignerHttpPort({
      endpoint: `${origin}/signer`,
...
test("disabled by default: configured clients perform no RPC or signer HTTP writes", async (t) => {
  ...
  const disabled: Parameters<typeof createRegistryAdapter>[0] = { ...f.options }
  delete disabled.writeEnabled
...
test("viem sends exact ABI bytes to the managed broker and verifies canonical digest with three confirmations", async (t) => {
  ...
  const decoded = decodeFunctionData({
    ...
    data: command.data as `0x${string}`
  })
  ...
  assert.deepEqual(decoded.args, [
    ...
    `0x${"ab".repeat(32)}`,
    `0x${"ab".repeat(32)}`,
    "0x0190f7b07c4b7e3a8f12123456789abc",
    1,
    BigInt(Date.parse(request.publishedAt) / 1000)
  ])
  assert.equal(String(command.data).slice(0, 10), "0x85eecf4c")
  assert.equal(
    f.calls.includes("eth_sendTransaction") || f.calls.includes("eth_sendRawTransaction"),
    false
  )
})
...
for (const scenario of [
  ...
  test(`${scenario} read-back can never report VERIFIED`, async (t) => {
    ...
    if (scenario === "wrong-contract")
      f.state.transactionOverrides.to = "0x0000000000000000000000000000000000000002"
...
test("same request retries coalesce and changed calldata cannot reuse the local or durable broker key", async (t) => {
  ...
  assert.equal(
    (await adapter.attest({ ...request, publishedAt: "2026-09-13T00:00:00Z" })).status,
    "DENIED"
  )
  assert.equal((await createRegistryAdapter(f.options).attest(request)).status, "VERIFIED")
  assert.equal(
    (
      await createRegistryAdapter(f.options).attest({
        ...request,
...
test("managed HTTP signer itself defaults disabled and receipt tracking remains readable after rollback", async (t) => {
  const f = await fixture(t)
  const disabledSigner = createManagedSignerHttpPort({
    endpoint: `${f.origin}/signer`,
    allowLoopbackHttp: true
  })
  assert.equal(
    (await createRegistryAdapter({ ...f.options, signer: disabledSigner }).attest(request)).status,
    "DENIED"
  )
  assert.equal(f.calls.includes("broker.submit"), false)
  let enabled = true
  const adapter = createRegistryAdapter({ ...f.options, writeEnabled: () => enabled })
  assert.equal((await adapter.attest(request)).status, "VERIFIED")
  enabled = false
  assert.equal((await adapter.attest(request)).status, "DISABLED")
  assert.equal((await adapter.confirm(request, TRANSACTION)).status, "VERIFIED")
  assert.equal(f.actualTransactions, 1)
})

test("request bytes remain bound when a caller mutates its object during an HTTP round trip", async (t) => {
  const f = await fixture(t)
  const input = { ...request }
  const pending = createRegistryAdapter(f.options).attest(input)
  input.idempotencyKey = "changed-while-pending"
  input.manifestDigest = `sha256:${"cd".repeat(32)}`
  assert.equal((await pending).status, "VERIFIED")
  assert.equal(f.submissions[0]?.idempotencyKey, request.idempotencyKey)
})
...
test("production transport rejects insecure endpoints and embedded credentials", () => {
  assert.throws(
    () => createManagedSignerHttpPort({ endpoint: "http://signer.example.test" }),
    /ENDPOINT/u
  )
  assert.throws(
    () => createManagedSignerHttpPort({ endpoint: "https://user:secret@signer.example.test" }),
    /ENDPOINT/u
  )
})
