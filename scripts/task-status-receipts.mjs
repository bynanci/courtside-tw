...
const TASK_STATUS_RECEIPT_PATH =
...
const taskReceiptSha = (v) =>
  typeof v === "string" && /^[a-f0-9]{40}$/u.test(v);
const taskReceiptSha256 = (v) =>
  ...
  function value(depth) {
    ...
    if (token === "{" || token === "[") {
      ...
      while (offset < text.length) {
        ...
        if (text[offset++] !== ",")
          throw new Error("invalid JSON entry separator");
      }
...
function taskReceiptContract(body) {
  ...
  if (!/^```json\s+[\s\S]+\s+```$/u.test(block))
    throw new Error("invalid authorization JSON fence");
  return parseTaskReceiptJson(
    block.replace(/^```json\s+/u, "").replace(/\s+```$/u, ""),
  );
}

function taskStatusSignatures(text) {
  if (typeof text !== "string")
    throw new Error("canonical tasks text is missing");
  ...
  if (
    ...
    throw new Error(
      "canonical task IDs must remain exactly T001–T112 in order",
    );
  const checkbox = taskReceiptHash(
    rows
      .map(
        (row) => `${row.id}:${row.checked.toLowerCase() === "x" ? "1" : "0"}`,
      )
      .join("\n"),
  );
  if (
    ...
    throw new Error(
      "canonical checkbox signature must remain 86 complete and 26 pending",
    );
...
function validateTaskStatusReceipt(input = {}) {
  ...
  try {
    ...
    need(
      mode === "CANDIDATE" || mode === "ACCEPTED_BASE",
    ...
    need(
      r.schema_version === TASK_STATUS_RECEIPT_SCHEMA &&
        r.repository === "bynanci/courtside-tw",
    ...
    need(
      taskReceiptEqual(r.scope_boundaries, TASK_STATUS_BOUNDARIES),
      "scope boundaries changed",
    );
    need(
      taskReceiptSamePaths(
    ...
    for (const p of TASK_STATUS_DOCUMENT_PATHS) {
      ...
      need(
        taskReceiptSamePaths(Object.keys(d ?? {}), [
          "before_sha256",
          "after_sha256",
        ]) &&
    ...
    need(
      ref &&
        ...
        a?.issue_url ===
          `https://api.github.com/repos/bynanci/courtside-tw/issues/${ref?.[1]}` &&
    ...
    const expected = {
      ...r,
      schema_version:
        "courtside-task-status-reconciliation-owner-authorization/v1",
    ...
    const oldTasks = taskStatusSignatures(
      before?.[TASK_STATUS_DOCUMENT_PATHS[1]],
    );
    const newTasks = taskStatusSignatures(
      after?.[TASK_STATUS_DOCUMENT_PATHS[1]],
    );
    need(
    ...
    need(
      taskReceiptEqual(
        ...
        Array.from(
          { length: 17 },
          (_, i) => `UIR-${String(i + 1).padStart(3, "0")}`,
        ),
    ...
    for (let i = 0; i < oldUi.length; i++) {
      if (oldUi[i][2] !== "UIR-017")
        need(
          oldUi[i][0] === newUi[i]?.[0],
          "only UIR-017 may change in the UI task rows",
        );
      else {
        const definition =
...
function validateTaskReceiptBootstrap(input = {}) {
  ...
  need(
    a?.status === "VERIFIED" &&
      ...
      a?.issue_url ===
        "https://api.github.com/repos/bynanci/courtside-tw/issues/196" &&
      a?.user_login === "bynanci" &&
      a?.author_association === "OWNER" &&
...
function fetchTaskReceiptJson(url, environment) {
  return parseTaskReceiptJson(
    execFileSync(
      process.execPath,
      ["--input-type=module", "--eval", taskReceiptFetchScript, url],
      {
        encoding: "utf8",
        env: {
...
function inspectTaskReceiptHistory({
  git,
  fetchJson,
  api,
  base,
  ...
  if (localHead) {
    ...
    if (shas.length < 1 || shas.length > 250)
      throw new Error(
        "reviewed receipt history is empty or exceeds 250 commits",
      );
    commits = shas.map((sha) => {
      const [parents, authoredAt, committedAt] = git([
        "show",
        "-s",
        "--format=%P%n%aI%n%cI",
        sha,
    ...
    for (let page = 1; page <= 3; page++) {
      const comparison = fetchJson(
        `${api}/compare/${base}...${head}?per_page=100&page=${page}`,
      );
      if (
        ...
        throw new Error(
          "reviewed receipt comparison is incomplete or lacks base ancestry",
        );
    ...
    commits = commits.map((commit) => {
      ...
      const detail = fetchJson(
        `${api}/commits/${commit.sha}?per_page=100&page=1`,
      );
      if (
        ...
        throw new Error(
          "reviewed receipt commit read-back differs from comparison",
        );
      return detail;
    });
  }
  ...
  for (const commit of commits) {
    if (
      ...
      throw new Error(
        "reviewed receipt history is not a linear sequence from the authorized base",
      );
    if (
      !Array.isArray(commit.files) ||
      commit.files.length < 1 ||
      commit.files.length > allowedPaths.length ||
      new Set(commit.files.map((file) => file?.filename)).size !==
        commit.files.length ||
      commit.files.some(
        (file) =>
      ...
      throw new Error(
        "reviewed receipt history contains changes outside the authorized closure",
      );
    if (
  ...
  if (previous !== head)
    throw new Error(
      "reviewed receipt history does not close at the original PR head",
    );
  ...
  try {
    ...
    if (bootstrapRequested) {
      ...
      if (
        ...
        number = parseTaskReceiptJson(
          fs.readFileSync(environment.GITHUB_EVENT_PATH, "utf8"),
        )?.pull_request?.number;
      ...
      const input = {
        ...
        commitsAfterAuthorization: reviewedHistory.commits.every((commit) =>
          [commit.commit.author.date, commit.commit.committer.date].every(
            (time) =>
              Date.parse(time) > Date.parse(TASK_RECEIPT_BOOTSTRAP_TIME),
          ),
        ),
      ...
      return {
        mode: "BOOTSTRAP",
        input,
        binding: { head: currentHead, base: baseSha },
      };
    }
    ...
    const authorization = normalizeTaskReceiptComment(
      fetchJson(`${api}/issues/comments/${ref[1]}`),
    );
    ...
    const mode = unchangedReceipt
      ? "ACCEPTED_BASE"
      : protectedPush
        ? "RECEIPT_PUSH"
        : "CANDIDATE";
    const sourceHead =
      mode === "CANDIDATE" ? currentHead : pullRequest.merge_commit_sha;
    if (!taskReceiptSha(sourceHead))
      throw new Error("merged receipt source commit is unavailable");
    const reviewedHistory = inspectTaskReceiptHistory({
      git,
      fetchJson,
      api,
    ...
    if (mode !== "CANDIDATE") {
      ...
      const reviewedTree = fetchJson(
        `${api}/git/commits/${pullRequest.head.sha}`,
      )?.tree?.sha;
      if (git(["rev-parse", `${sourceHead}^{tree}`]).trim() !== reviewedTree)
        throw new Error("merged receipt tree differs from the reviewed PR");
      if (
        !taskReceiptEqual(
          git(["show", "-s", "--format=%P", sourceHead]).trim().split(" "),
          [receipt.base_sha],
        )
      )
        throw new Error(
          "merged receipt is not its reviewed single-parent squash",
        );
      if (
        mode === "RECEIPT_PUSH" &&
        ...
        throw new Error(
          "receipt push is not its exact reviewed single-parent squash",
        );
    }
    if (
      !regular(sourceHead, [
    ...
    const sourcePaths =
      mode === "CANDIDATE" ? changedPaths : diff(receipt.base_sha, sourceHead);
    const input = {
      ...
      pullRequest,
      mergedUiPullRequest,
      uiMergeAncestorOfBase: ancestor(
        receipt.completion_evidence?.merge_sha,
        receipt.base_sha,
      ),
    ...
    return {
      mode: "INVALID",
      errors: [`task-status receipt inspection: ${error.message}`],
    };
  }
}
...
function validateTaskReceiptPolicy(
  ...
  if (
    ...
    return fail(
      "authenticated receipt content and clean current Git binding are required",
    );
  const result =
  ...
  return {
    ...result,
    mode: readback.mode,
    allowedPaths: [...readback.input.changedPaths],
  };
}
