...
test("trusted classifier recognizes a fully authenticated documentation receipt without treating prose as a release candidate", () => {
  ...
  assert.equal(
    sandbox.classifyCandidate(195, rows, rows.length, verdict),
    "NOT_APPLICABLE",
  );
});
...
test("an inherited receipt authenticates only its sealed snapshot and never grants current changed paths", () => {
  ...
  assert.equal(
    sandbox.classifyCandidate(
      ...
      [
        {
          filename: "specs/001-taiwan-basketball-magazine-ebook/tasks.md",
          status: "modified",
        },
      ],
      1,
  ...
  const renamed = rows.map((row, i) =>
    i
      ...
      : {
          ...row,
          status: "renamed",
          previous_filename: "scripts/validate-beta-release.mjs",
        },
  );
  assert.equal(
    sandbox.classifyCandidate(195, renamed, renamed.length, verdict),
    "T086",
  );
  assert.equal(
    sandbox.classifyCandidate(195, rows, rows.length + 1, verdict),
    "UNKNOWN",
  );
  assert.equal(
    sandbox.classifyCandidate(161, rows, rows.length, verdict),
    "T086",
  );
});
...
test("trusted workflow carries exactly the reviewed pure receipt validator and parses without candidate code", () => {
  const module = fs.readFileSync(
    new URL("../task-status-receipts.mjs", import.meta.url),
    "utf8",
  );
...
test("a malformed receipt without canonical tasks edits still requires release classification", () => {
  ...
  assert.equal(
    sandbox.classifyCandidate(195, receiptOnly, 1, { status: "FAIL" }),
    "T086",
  );
  assert.equal(
...
function liveGate(fixture, mutate = () => {}) {
          ...
          data: {
            ...
            content: Buffer.from(
              path === docs[3]
                ...
                : (ref === f.protectedMainSha
                    ? f.baseDocuments
                    : f.targetDocuments)[path],
            ).toString("base64"),
          },
        }),
  ...
  return {
    state,
    read: () =>
      context.taskDocumentationRead(state.pr, state.mainSha, state.files),
...
test("irrelevant contributors and ordinary new OWNER comments never enter global reconciliation", () => {
  ...
  assert.equal(
    commentJobRuns(ownerEvent("created", 196, "ordinary discussion")),
    false,
  );
  assert.equal(
    commentJobRuns(
      ownerEvent("created", 999, "<!-- t086:owner-adjudication:start -->"),
    ),
    false,
  );
  assert.equal(
    commentJobRuns(
      ownerEvent("created", 999, "<!-- t086:owner-authorization:start -->"),
    ),
    false,
  );
...
test("only admitted jobs share concurrency, so ignored comments cannot replace a pending invalidation", () => {
  ...
  assert.equal(
    concurrency?.group,
    "t086-required-gate-" + "$" + "{{ github.repository }}",
  );
  assert.equal(concurrency?.["cancel-in-progress"], false);
  assert.deepEqual(parsedWorkflow.on.issue_comment.types, [
    "created",
    "edited",
    "deleted",
  ]);
...
for (const count of [31, 250]) {
  test(
    "unpaginated GitHub Compare verifies the complete bounded " +
      count +
      "-commit history",
    async () => {
      const gate = liveGate(taskStatusFixture(), (state) =>
