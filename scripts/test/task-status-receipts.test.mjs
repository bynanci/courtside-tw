...
function fixture() {
  const baseDocuments = Object.fromEntries(
    ["README.md", tasksPath, ledgerPath].map((p) => [
      p,
      fs.readFileSync(new URL(p, root), "utf8"),
    ]),
  );
  const targetDocuments = {
    ...baseDocuments,
    "README.md":
      baseDocuments["README.md"] +
  ...
  const receipt = {
    ...
    pull_request: 195,
    authorization_ref:
      "https://github.com/bynanci/courtside-tw/issues/196#issuecomment-6000000000",
    documents: Object.fromEntries(
      Object.keys(baseDocuments).map((p) => [
        p,
        {
          before_sha256: hash(baseDocuments[p]),
          after_sha256: hash(targetDocuments[p]),
        },
      ]),
    ),
...
function authorizeTaskStatusFixture(f) {
  f.receipt.documents = Object.fromEntries(
    Object.keys(f.baseDocuments).map((p) => [
      p,
      {
        before_sha256: hash(f.baseDocuments[p]),
        after_sha256: hash(f.targetDocuments[p]),
      },
    ]),
  );
  f.receipt.canonical_checkbox_signature_sha256 = signature(
    f.baseDocuments[tasksPath],
  );
  f.receipt.task_definition_signature_sha256 = definitions(
    f.baseDocuments[tasksPath],
  );
  const contract = {
    ...f.receipt,
...
test("task signatures include indented continuation bytes under each task", () => {
  const f = fixture();
  const baseline = receiptModule.taskStatusSignatures(
    f.baseDocuments[tasksPath],
  );
...
test("exports a sealed bootstrap validator for the six authorized implementation paths", () => {
  ...
  assert.equal(
    receiptModule.TASK_RECEIPT_BOOTSTRAP_PATHS.includes(tasksPath),
    false,
  );
});
...
function realReceiptHistory(
  ...
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), "courtside-receipt-history-"),
  );
  ...
  if (bootstrap) {
    ...
    git(
      ...
      "-B",
      "receipt-review-main",
      receiptModule.TASK_RECEIPT_BOOTSTRAP_BASE,
    );
  ...
  const branch = bootstrap
    ? receiptModule.TASK_RECEIPT_BOOTSTRAP_BRANCH
    : f.receipt.branch;
  const paths = bootstrap
    ? [...receiptModule.TASK_RECEIPT_BOOTSTRAP_PATHS]
    : f.changedPaths;
  const runtimeBefore = fs.readFileSync(
    path.join(directory, runtimePath),
    "utf8",
  );
  git("switch", "-C", branch);
  if (hiddenRuntime) {
  ...
  if (bootstrap) {
    ...
    for (const [name, text] of Object.entries(f.targetDocuments))
      put(name, text);
      ...
      files: git(
        ...
        .map((row) => {
          ...
          return {
            filename,
            status:
              { A: "added", M: "modified", D: "removed" }[status] ?? status,
          };
        }),
    }));
  ...
  const rawComment = {
    ...f.authorization,
    user: { login: f.authorization.user_login },
  };
  ...
  const fetchJson = (url) => {
    ...
    else if (url.endsWith(`/git/commits/${reviewed}`))
      data = { tree: { sha: reviewedTree } };
...
test("a deleted reviewed branch uses complete API history and still accepts reviewed pre-approval documents", (t) => {
  ...
  assert.ok(
    f.state.requests.some((url) =>
      /\/commits\/[a-f0-9]{40}\?per_page=100/u.test(url),
    ),
  );
});
...
for (const mode of ["ACCEPTED_BASE", "RECEIPT_PUSH", "BOOTSTRAP"]) {
  ...
  test(`real Git ${mode} cannot use a later squash time to repair an unauthorized original closing time`, (t) => {
    const f = realReceiptHistory(t, {
      ...
      reviewedDate:
        mode === "BOOTSTRAP"
          ? bootstrapFixture().authorization.created_at
...
test("bootstrap rejects an earlier implementation commit even when its final reviewed commit follows approval", (t) => {
  const f = realReceiptHistory(t, {
    bootstrap: true,
    preparedBeforeAuthorization: true,
  });
  assert.equal(f.inspect("BOOTSTRAP").status, "FAIL");
});
...
test("API reviewed-history pagination and commit-file readback fail closed", async (t) => {
  ...
  f.state.commits = Array.from({ length: 101 }, (_, index) => ({
    ...
    sha:
      index === 100 ? f.reviewed : (index + 1).toString(16).padStart(40, "0"),
    parents: [
      { sha: index === 0 ? f.base : index.toString(16).padStart(40, "0") },
    ],
    files:
      index === 100
        ? original.at(-1).files
  ...
  assert.equal(
    f.state.requests.filter((url) => url.includes("/compare/")).length,
    2,
  );
  assert.equal(
    f.state.requests.filter((url) => /\/commits\/[a-f0-9]{40}\?/u.test(url))
      .length,
    101,
  );
  for (const [name, mutate] of [
    ...
    [
      ...
      (url, data) => {
        if (url.includes("/compare/"))
          data.merge_base_commit.sha = "e".repeat(40);
      },
    ],
    [
      ...
      (url, data) => {
        if (url.includes("/compare/"))
          for (const commit of data.commits)
            commit.parents = [{ sha: "e".repeat(40) }];
        if (/\/commits\/[a-f0-9]{40}\?/u.test(url))
          data.parents = [{ sha: "e".repeat(40) }];
      },
    ],
    [
      ...
      (url, data) => {
        if (url.includes("/compare/"))
          for (const commit of data.commits)
            commit.parents.push({ sha: "e".repeat(40) });
        if (/\/commits\/[a-f0-9]{40}\?/u.test(url))
          data.parents.push({ sha: "e".repeat(40) });
      },
    ],
    [
    ...
    [
      ...
      (url, data) => {
        if (/\/commits\/[a-f0-9]{40}\?/u.test(url))
          data.commit.committer.date = "invalid";
      },
    ],
  ]) {
    await t.test(name, () => {
