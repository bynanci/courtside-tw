...
# evidence; the upgrade itself must preserve the active/previous pointers and
...
expect_failure "unconfirmed v1 state upgrade" env DATABASE_URL="postgresql://$SENTINEL.invalid/courtside" \
  python3 "$CONTROLLER" \
    --state "$LEGACY_STATE" \
    --environment test \
    --receipt "$WORK_DIR/legacy-upgrade-denied.json" \
    upgrade-state --activation-history "$UPGRADE_EVIDENCE" --confirmation WRONG
env DATABASE_URL="postgresql://$SENTINEL.invalid/courtside" \
  python3 "$CONTROLLER" \
    --state "$LEGACY_STATE" \
    --environment test \
    --receipt "$WORK_DIR/legacy-upgrade.json" \
    upgrade-state --activation-history "$UPGRADE_EVIDENCE" \
...
assert state["environment"] == "test", state
assert state["activated_releases"] == ["release-a", "release-b"], state
assert state["last_action_receipt"]["action"] == "upgrade-state", state
assert state["receipt_history"][0]["action"] == "activate", state
assert state["receipt_history"][-1]["action"] == "upgrade-state", state
assert receipt["from_schema_version"] == 1, receipt
assert receipt["to_schema_version"] == 2, receipt
print("verify-deployment-rollback: pass - v1 ledger upgraded with preserved history")
PY
...
receipt = {
    ...
    "state_upgrade": {
        "from_schema_version": upgrade["from_schema_version"],
        "to_schema_version": upgrade["to_schema_version"],
        "environment": upgrade["environment"],
        "preserved_receipt_actions": [
            "activate",
            upgrade["action"],
        ],
    },
    "negative_paths": [
        ...
        "v1 state upgrade requires explicit environment and activation history",
        "v1 state upgrade preserves bounded receipt history",
