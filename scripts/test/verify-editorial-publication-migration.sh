...
PROJECT_NAME="courtside-v004-$$"
TIMEOUT_SECONDS="${V004_TIMEOUT_SECONDS:-120}"
...
for migration in \
  ...
  "$REPO_ROOT/apps/api/src/main/resources/db/migration/V004__editorial_publication_workflow.sql"; do
  [ -r "$migration" ] || fail "missing migration: $migration"
  sql -f - < "$migration"
done
echo "verify-editorial-publication-migration: V001/V002/V003/V004 applied"
...
expect_failure "duplicate idempotency key" "INSERT INTO publication_idempotency (actor_subject, operation, idempotency_key, request_hash_sha256, response) VALUES ('migration-editor', 'SUBMIT', 'migration-v004-key', repeat('d', 64), '{}'::jsonb);"
...
INSERT INTO public.publication_snapshot (
    aggregate_type, aggregate_id, snapshot_version, content_document, checksum_sha256, created_by
) VALUES (
    'ARTICLE',
