...
const requiredGateBase = "c2a4b2d2a5ba3f97fc3c50b7723c01f4d3b98e2d"
const requiredGateBranch = "fix/t086-check-lifecycle-c2a4b2d"
...
const requiredGateAuthorizationBody = [
  "<!-- t086-required-gate:owner-exact-path-dispatch:v8:start -->",
  "## Fresh OWNER exact-base dispatch — #164 check-run lifecycle repair",
  "",
  "**Decision:** `DISPATCH_ACCEPTED / IMPLEMENTATION_REVIEW_OPTIMIZE_MERGE_ENGINEERING_ONLY`.",
  "",
  "This immutable successor is bound to the protected main after PR #182. It authorizes only the observed GitHub Checks API lifecycle repair: an in-progress update must omit `conclusion`; completed invalidation must retain an explicit failure conclusion. It does not refresh PR #161 or authorize provider/ruleset mutation.",
  "",
  "```json",
  "{",
  '  "schema_version": "courtside-t086-required-gate-owner-exact-path-dispatch/v6",',
  '  "decision": "DISPATCH_ACCEPTED",',
  '  "accepted_by": "bynanci",',
  '  "repository": "bynanci/courtside-tw",',
  '  "tracker_issue": 164,',
  '  "pr": 183,',
  '  "branch": "fix/t086-check-lifecycle-c2a4b2d",',
  '  "authorization_base": {',
  '    "branch": "main",',
  '    "sha": "c2a4b2d2a5ba3f97fc3c50b7723c01f4d3b98e2d",',
  '    "parent_sha": "b8f092534f7cb3d550738be2d66ee1d96d676980",',
  '    "tree_sha": "d68b05816e46c34f49c59ac2b228717c719c5e36",',
  '    "protected": true,',
  '    "t086_push": {',
  '      "run_id": 34341321199,',
  '      "result": "FAIL",',
  '      "reason": "Checks API rejected null conclusion on in-progress update"',
  "    },",
  '    "ci": {',
  '      "run_id": 34341321233,',
  '      "result": "PENDING"',
  ...
  "  },",
  '  "initial_seed": {',
  '    "head_sha": "baf2e0fae0cc64eb932f4e7b4ac5b9b08c8381da",',
  '    "tree_sha": "bc7c45f8b64694088181b1f4c38dfb3ab2f69986",',
  '    "changed_paths": [',
  '      "scripts/test/validate-traceability.test.mjs"',
  "    ],",
  '    "role": "accepted pre-dispatch tests-first RED seed; no implementation or release behavior"',
  "  },",
  '  "authorized_paths": [',
  '    ".github/workflows/t086-required-gate.yml",',
  '    "scripts/validate-traceability.mjs",',
  ...
  "  ],",
  '  "allowed_actions": [',
  '    "append the causal check-run lifecycle test and trusted workflow repair in the three exact paths",',
  '    "run exact-head non-deploying CI, Security, independent review and optimize",',
  '    "mark PR #183 ready only after exact-head evidence and review pass",',
  '    "perform one protected exact-head squash merge of PR #183 after all required checks and zero unresolved threads"',
  "  ],",
  '  "invariants": [',
  '    "all frozen T085 receipt bytes, README bytes and task checkbox bytes remain unchanged",',
  '    "no beta flag removal, T086 PASS claim, release acceptance or risk adjudication",',
  ...
  '    "in-progress check updates omit conclusion while completed invalidation uses only an allowed conclusion"',
  "  ],",
  ...
  "",
  "- The exact current-main base and three-path closure must remain unchanged.",
  "- RED evidence must remain attributable to the named seed; GREEN evidence must bind the final exact head.",
  "- No GitHub Checks API request may send a null conclusion; completed invalidations must use an allowed conclusion.",
  ...
  "",
  "<!-- t086-required-gate:owner-exact-path-dispatch:v8:end -->"
].join("\n")
function makeRequiredGateReadback(overrides = {}) {
  return {
    ...
    created_at: "2026-09-09T10:41:33Z",
    updated_at: "2026-09-09T10:41:33Z",
...
function makeRequiredGateActionsContext(
  root,
  { eventOverrides = {}, environmentOverrides = {} } = {}
) {
...
function runRequiredGateFixture(fixture, overrides = {}) {
  ...
  return runCompletedFixture(fixture, {
    ...
    evaluatedHeadCommittedAt: "2026-09-09T11:00:00.000Z",
...
test("issue 164 protected-main push rejects replayed topology", () => {
  ...
  for (const [label, binding] of [
    ...
    ["tree drift", { head_tree_sha: "not-a-tree" }]
  ]) {
...
test("issue 164 rejects missing, stale, spoofed, edited and widened OWNER authority", () => {
  ...
  for (const [label, readback] of [
    ...
    [
      "body",
      makeRequiredGateReadback({
        body: requiredGateAuthorizationBody.replace(
          '"decision": "DISPATCH_ACCEPTED"',
...
test("issue 164 rejects scope, base, exact-head, draft, PR, fork, Actions and topology replays", () => {
  ...
  for (const [label, overrides] of [
    ...
    [
      "different workflow",
      { changedPaths: [".github/workflows/unrelated.yml", ...requiredGatePaths.slice(1)] }
    ],
    ...
    ["pre-dispatch head", { evaluatedHeadCommittedAt: "2026-09-09T10:41:33Z" }],
    [
      "ready",
      {
        githubActionsContext: makeRequiredGateActionsContext(fixture.root, {
          eventOverrides: { draft: false }
        })
      }
    ],
    [
    ...
    [
      ...
      {
        githubActionsContext: makeRequiredGateActionsContext(fixture.root, {
          environmentOverrides: { GITHUB_REF: "refs/pull/172/merge" }
        })
      }
    ],
    [
      ...
      {
        githubActionsContext: makeRequiredGateActionsContext(fixture.root, {
          eventOverrides: { number: 161 },
          environmentOverrides: { GITHUB_REF: "refs/pull/161/merge" }
        })
      }
    ],
    [
        ...
        githubActionsContext: makeRequiredGateActionsContext(fixture.root, {
          eventOverrides: {
            head: {
              sha: fixtureReceiptHead,
              ref: requiredGateBranch,
              repo: { full_name: "attacker/courtside-tw" }
            }
          }
        })
      }
    ...
    [
      ...
      {
        githubActionsContext: makeRequiredGateActionsContext(fixture.root, {
          environmentOverrides: { GITHUB_HEAD_REF: "other" }
        })
      }
    ],
    ...
    [
      "merge ancestry",
      {
        gitBinding: {
          ...
          change_base_sha: requiredGateBase,
          change_base_ancestor: true,
          required_gate_commit_count: 2,
          required_gate_merge_commit_count: 1
        }
      }
    ],
    [
...
test("issue 164 CLI inspection reads only the exact authorized scope and pinned comment", () => {
  ...
  const report = traceabilityValidator.inspectRequiredGateAuthorizationForState(
    fixture.root,
    options
  )
  ...
  for (const overrides of [
    ...
    assert.equal(
      traceabilityValidator.inspectRequiredGateAuthorizationForState(fixture.root, {
        ...options,
        ...overrides
      }),
      null
...
function makePostT085MaintenanceActionsContext(
  ...
  const eventPath = path.join(root, "github-post-t085-maintenance-event.json")
  const pullRequestEvent = {
    number: eventPullRequest,
    head: { sha: fixtureReceiptHead, ref: headRef },
    base: { sha: baseSha, ref: "main" }
  }
  if (pullRequestDraft !== null) pullRequestEvent.draft = pullRequestDraft
  fs.writeFileSync(
    eventPath,
    JSON.stringify({
      repository: { full_name: "bynanci/courtside-tw" },
  ...
  return traceabilityValidator.inspectGitHubActionsContext({
    ...
    gitBinding: {
      head: fixtureReceiptHead,
      change_base_sha: baseSha,
      change_base_ancestor: true
    }
  })
}

function makeAndroidNativeSurfaceActionsContext(
  root,
  {
    pullRequest = 169,
    eventPullRequest = pullRequest,
    headRef = androidNativeSurfaceBranch,
    baseSha = androidNativeSurfaceAuthorizationBaseSha,
    githubRef = `refs/pull/${pullRequest}/merge`,
    pullRequestDraft = true
  } = {}
) {
  const eventPath = path.join(root, "github-android-native-surface-event.json")
  const pullRequestEvent = {
    number: eventPullRequest,
    head: { sha: fixtureReceiptHead, ref: headRef },
    base: { sha: baseSha, ref: "main" }
...
function makeAndroidNativeSurfacePushActionsContext(
  root,
  { before = androidNativeSurfaceAuthorizationBaseSha, after = androidNativeSurfaceMergeSha } = {}
) {
...
function makePostT085MaintenancePushActionsContext(root) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      repository: { full_name: "bynanci/courtside-tw" },
      before: postT085MaintenanceAuthorizationBaseSha,
      after: fixtureReceiptHead,
      ref: "refs/heads/main"
    })
  )
...
function writeExactHeadForActionsContext(root, context) {
  fs.writeFileSync(
    path.join(root, "artifacts/exact-head.json"),
    JSON.stringify({
      source_head_sha: context.source_head_sha,
      expected_source_head: context.source_head_sha,
      source_event: context.event_name,
      ...
      github_base_ref: context.base_ref
    })
  )
}
...
function makePostT085MaintenanceAuthorizationReadback({
  ...
  return {
    ...
    updated_at: postT085MaintenanceAuthorizationRecordedAt,
    body: [
      "<!-- post-t085-maintenance:owner-authorization:start -->",
      "```json",
      JSON.stringify(authorization),
      "```",
      "<!-- post-t085-maintenance:owner-authorization:end -->"
    ].join("\n"),
    errors: [],
    ...readbackOverrides
  }
...
function makeAndroidNativeSurfaceAuthorizationReadback({
  ...
  const dispatchAuthorization = {
    ...
    authorized_paths: androidNativeSurfaceAuthorizedPaths.slice(0, 2),
    authorized_actions: [
  ...
  const addendumAuthorization = {
    schema_version: "courtside-android-native-surface-traceability-addendum/v1",
    ...
    scope_proposal_ref: androidNativeSurfaceScopeProposalRef,
    pull_request: 169,
    authorization_base: {
      branch: "main",
      sha: androidNativeSurfaceAuthorizationBaseSha,
    ...
    seed_head: {
      branch: androidNativeSurfaceBranch,
      sha: androidNativeSurfaceSeedHeadSha
    },
    ...
    authorized_amendment: {
      parent_sha: androidNativeSurfaceSeedHeadSha,
      maximum_commit_count: 2,
      changed_paths: [...androidNativeSurfaceAmendmentPaths],
      tests_first: true
    },
    authorized_actions: [
  ...
  return {
    dispatch: {
      status: "VERIFIED",
      source: "github-api",
      html_url: androidNativeSurfaceDispatchRef,
      issue_url: "https://api.github.com/repos/bynanci/courtside-tw/issues/121",
      user_login: fixtureReceiptOwner,
      author_association: "OWNER",
      created_at: androidNativeSurfaceDispatchRecordedAt,
      updated_at: androidNativeSurfaceDispatchRecordedAt,
      body: [
        "<!-- android-native-surface:owner-dispatch:v1:start -->",
        "```json",
        JSON.stringify(dispatchAuthorization),
        "```",
        "<!-- android-native-surface:owner-dispatch:v1:end -->"
      ].join("\n"),
      errors: [],
      ...dispatchReadbackOverrides
    },
    addendum: {
      status: "VERIFIED",
      source: "github-api",
      html_url: androidNativeSurfaceAuthorizationRef,
      issue_url: "https://api.github.com/repos/bynanci/courtside-tw/issues/121",
      user_login: fixtureReceiptOwner,
      author_association: "OWNER",
      created_at: androidNativeSurfaceAuthorizationRecordedAt,
      updated_at: androidNativeSurfaceAuthorizationRecordedAt,
      body: [
...
function makePost169GovernanceAuthorizationReadback({
  dispatchAuthorizationOverrides = {},
  addendumAuthorizationOverrides = {},
  formatAuthorizationOverrides = {},
  reviewRemediationAuthorizationOverrides = {},
  finalSealAuthorizationOverrides = {},
  squashPushSupersessionAuthorizationOverrides = {},
  finalSealV4AuthorizationOverrides = {},
  dispatchReadbackOverrides = {},
  addendumReadbackOverrides = {},
  formatReadbackOverrides = {},
  reviewRemediationReadbackOverrides = {},
  finalSealReadbackOverrides = {},
  ...
  const formatAuthorization = {
    schema_version: "courtside-post169-governance-reconciliation-format-addendum/v1",
    decision: "SAME_FOUR_PATH_FORMAT_REMEDIATION_ACCEPTED",
    accepted_by: fixtureReceiptOwner,
    repository: "bynanci/courtside-tw",
    issue: "https://github.com/bynanci/courtside-tw/issues/121",
    dispatch_ref: post169GovernanceDispatchRef,
    prior_addendum_ref: post169GovernanceAddendumRef,
    pull_request: post169GovernancePullRequest,
    authorization_base: {
      branch: "main",
      sha: post169GovernanceAuthorizationBaseSha,
    ...
    authorized_amendment: {
      parent_sha: post169GovernanceSeedHeadSha,
      commit_count: 1,
      changed_paths: [...post169GovernanceAuthorizedPaths],
  ...
  const reviewRemediationAuthorization = {
    schema_version: "courtside-post169-governance-reconciliation-review-remediation-addendum/v1",
    decision: "REVIEW_REMEDIATION_ACCEPTED",
    accepted_by: fixtureReceiptOwner,
    repository: "bynanci/courtside-tw",
    issue: "https://github.com/bynanci/courtside-tw/issues/121",
    dispatch_ref: post169GovernanceDispatchRef,
    proposal_ref: post169GovernanceReviewProposalRef,
    pull_request: post169GovernancePullRequest,
    authorization_base: {
      branch: "main",
      sha: post169GovernanceAuthorizationBaseSha,
    ...
    blocked_head: {
      sha: post169GovernanceBlockedHeadSha,
      tree_sha: post169GovernanceBlockedTreeSha
    },
    tests_first_seed: {
      branch: post169GovernanceBranch,
      sha: post169GovernanceSeedHeadSha
  ...
  const finalSealAuthorization = {
    schema_version: "courtside-post169-governance-reconciliation-final-seal/v2",
    decision: "FINAL_HEAD_SEALED",
    accepted_by: fixtureReceiptOwner,
    repository: "bynanci/courtside-tw",
    issue: "https://github.com/bynanci/courtside-tw/issues/121",
    dispatch_ref: post169GovernanceDispatchRef,
    remediation_ref: post169GovernanceReviewRemediationRef,
    pull_request: post169GovernancePullRequest,
    authorization_base: {
      branch: "main",
      sha: post169GovernanceAuthorizationBaseSha,
    ...
    final_head: {
      branch: post169GovernanceBranch,
      sha: post169GovernancePriorFinalHeadSha,
      tree_sha: post169GovernancePriorFinalTreeSha,
      parent_sha: post169GovernanceSeedHeadSha,
      parent_count: 1,
      commit_count: 2,
    ...
    authorized_paths: [...post169GovernanceAuthorizedPaths],
    content_blobs: {
      "README.md": post169GovernanceBlockedReadmeBlobSha,
      [`${featurePath}/tasks.md`]: post169GovernanceBlockedTasksBlobSha,
      "scripts/test/validate-traceability.test.mjs": post169GovernancePriorFinalTestBlobSha,
      "scripts/validate-traceability.mjs": post169GovernancePriorFinalValidatorBlobSha
    },
    blocked_head_preservation: {
      blocked_head_sha: post169GovernanceBlockedHeadSha,
      readme_blob_sha: post169GovernanceBlockedReadmeBlobSha,
      tasks_blob_sha: post169GovernanceBlockedTasksBlobSha,
      task_checkbox_signature_sha256: post169GovernanceTaskCheckboxSignatureSha256
    },
    state: {
      draft: true,
      hold: true,
  ...
  const squashPushSupersessionAuthorization = {
    schema_version: "courtside-post169-governance-squash-push-final-supersession/v2",
    ...
    prior_final_head: {
      sha: post169GovernanceSupersededFinalHeadSha,
      tree_sha: post169GovernanceSupersededFinalTreeSha,
      final_seal_ref: post169GovernanceSupersededFinalSealRef
    },
    authorization_base: {
      branch: "main",
      sha: post169GovernanceAuthorizationBaseSha,
    ...
    tests_first_seed_sha: post169GovernanceSeedHeadSha,
    authorized_delta_paths: [
      "scripts/test/validate-traceability.test.mjs",
      "scripts/validate-traceability.mjs"
    ],
    required_result: {
      ...
      one_time: true,
      bypass: false,
      protected_main_push_parent_sha: post169GovernanceAuthorizationBaseSha
    },
    acceptance: [
  ...
  const finalSealV4Authorization = {
    schema_version: "courtside-post169-governance-final-seal/v4",
    decision: "FINAL_HEAD_SEALED__CONDITIONAL_SQUASH_MERGE",
    accepted_by: fixtureReceiptOwner,
    repository: "bynanci/courtside-tw",
    issue: "https://github.com/bynanci/courtside-tw/issues/121",
    supersession_ref: post169GovernanceSquashPushSupersessionRef,
    pull_request: post169GovernancePullRequest,
    authorization_base: {
      branch: "main",
      sha: post169GovernanceAuthorizationBaseSha,
    ...
    final_head: {
      branch: post169GovernanceBranch,
      sha: gitBinding.head,
      tree_sha: gitBinding.head_tree_sha,
      parent_sha: post169GovernanceSeedHeadSha,
      committed_at: gitBinding.head_committed_at
    },
    topology: { total_commits: 2, implementation_commits: 1, merge_commits: 0 },
    authorized_paths: [...post169GovernanceAuthorizedPaths],
    content_blobs: {
      "README.md": gitBinding.post169_governance_final_readme_blob_sha,
      [`${featurePath}/tasks.md`]: gitBinding.post169_governance_final_tasks_blob_sha,
      "scripts/test/validate-traceability.test.mjs":
        gitBinding.post169_governance_final_test_blob_sha,
      "scripts/validate-traceability.mjs": gitBinding.post169_governance_final_validator_blob_sha
    },
    task_checkbox_signature_sha256: gitBinding.post169_governance_task_checkbox_signature_sha256,
  ...
  return {
    ...
    formatAddendum: {
      status: "VERIFIED",
      source: "github-api",
      html_url: post169GovernanceFormatAddendumRef,
      issue_url: "https://api.github.com/repos/bynanci/courtside-tw/issues/121",
      user_login: fixtureReceiptOwner,
      author_association: "OWNER",
      created_at: post169GovernanceFormatAddendumRecordedAt,
      updated_at: post169GovernanceFormatAddendumRecordedAt,
      body: [
        "<!-- post169-governance-reconciliation:format-addendum:v1:start -->",
        "```json",
        JSON.stringify(formatAuthorization),
        "```",
    ...
    reviewRemediation: {
      status: "VERIFIED",
      source: "github-api",
      html_url: post169GovernanceReviewRemediationRef,
      issue_url: "https://api.github.com/repos/bynanci/courtside-tw/issues/121",
      user_login: fixtureReceiptOwner,
      author_association: "OWNER",
      created_at: post169GovernanceReviewRemediationRecordedAt,
      updated_at: post169GovernanceReviewRemediationRecordedAt,
      body: [
        "<!-- post169-governance-reconciliation:review-remediation-addendum:v1:start -->",
        "```json",
        JSON.stringify(reviewRemediationAuthorization),
        "```",
    ...
    finalSeal: {
      status: "VERIFIED",
      source: "github-api",
      html_url: post169GovernanceFinalSealRef,
      issue_url: "https://api.github.com/repos/bynanci/courtside-tw/issues/121",
      user_login: fixtureReceiptOwner,
      author_association: "OWNER",
      created_at: post169GovernanceFinalSealRecordedAt,
      updated_at: post169GovernanceFinalSealRecordedAt,
      body: [
        "<!-- post169-governance-reconciliation:final-seal:v2:start -->",
        "```json",
        JSON.stringify(finalSealAuthorization),
        "```",
        "<!-- post169-governance-reconciliation:final-seal:v2:end -->"
      ].join("\n"),
      errors: [],
      ...finalSealReadbackOverrides
    },
...
test("only the exact authenticated PR 169 squash push selects the frozen tasks digest", () => {
  for (const [name, contextOverrides, bindingOverrides, changedPaths] of [
    ...
    writeExactHeadForActionsContext(fixture.root, githubActionsContext)
    const head = contextOverrides.after ?? androidNativeSurfaceMergeSha
    const report = runAndroidNativeSurfaceFixture(fixture, {
      currentHead: head,
      githubActionsContext,
      post169GovernanceAuthorizationReadback: makePost169GovernanceAuthorizationReadback(),
    ...
    assert.equal(report.status, "FAIL", name)
    assert.match(report.errors.join("\n"), /tasks must match the accepted implementation snapshot/u)
  }
})
...
test("post-169 candidates always require the immutable exact-head OWNER seal", () => {
  const fixture = makeCompletedFixture()
  const tasksPath = path.join(fixture.root, featurePath, "tasks.md")
  const baseTasks = fs.readFileSync(tasksPath, "utf8")
  fs.writeFileSync(
    tasksPath,
    baseTasks.replace(/^- \[ \] T086 fixture$/m, `- [ ] T086 fixture${post169T086StatusSuffix}`)
  )
  fixture.changedPaths = [...post169GovernanceAuthorizedPaths]
  const gitBinding = makePost169GovernanceGitBinding()
  const report = runPost169GovernanceFixture(fixture, {
    gitBinding,
    post169GovernanceAuthorizationReadback: makePost169GovernanceAuthorizationReadback({
      gitBinding,
      finalSealReadbackOverrides: { status: "UNAVAILABLE" }
    })
  })

  assert.deepEqual(gitBinding.post169_governance_amendment_paths, post169GovernanceAmendmentPaths)
  assert.equal(report.status, "FAIL")
  assert.match(
    report.errors.join("\n"),
    /requires verified OWNER addendum and final-seal read-backs/
  )
})

test("authenticated PR 170 identity cannot bypass governance by restoring prose paths", () => {
  const fixture = makeCompletedFixture()
  ...
  assert.match(
    report.errors.join("\n"),
    /requires the exact four-path governance reconciliation scope/
  )
})

test("post-169 review remediation binds one immutable exact-head OWNER seal", () => {
  const fixture = makeCompletedFixture()
  const tasksPath = path.join(fixture.root, featurePath, "tasks.md")
  const baseTasks = fs.readFileSync(tasksPath, "utf8")
  fs.writeFileSync(
  ...
  const gitBinding = makePost169GovernanceGitBinding({
    head_committed_at: "2026-09-08T14:10:00.000Z",
    post169_governance_amendment_paths: [...post169GovernanceAuthorizedPaths]
  })
  const passing = runPost169GovernanceFixture(fixture, {
    evaluatedHeadCommittedAt: "2026-09-08T14:10:00.000Z",
    gitBinding,
  ...
  assert.equal(passing.status, "PASS", passing.errors.join("\n"))
  assert.equal(
    passing.source.post169_governance_authorization_readback.finalSeal.status,
    "VERIFIED"
  )

  const squashMergeSha = "3".repeat(40)
  const pushContext = makeAndroidNativeSurfacePushActionsContext(fixture.root, {
    before: post169GovernanceAuthorizationBaseSha,
    after: squashMergeSha
  })
  writeExactHeadForActionsContext(fixture.root, pushContext)
  const protectedMainPush = runPost169GovernanceFixture(fixture, {
    currentHead: squashMergeSha,
    evaluatedHeadCommittedAt: "2026-09-08T14:20:00.000Z",
    ...
    gitBinding: {
      ...gitBinding,
      head: squashMergeSha,
      head_committed_at: "2026-09-08T14:20:00.000Z",
      head_parent_sha: post169GovernanceAuthorizationBaseSha,
      head_parent_shas: [post169GovernanceAuthorizationBaseSha],
      post169_governance_seed_ancestor: false,
      post169_governance_amendment_commit_count: null,
      post169_governance_amendment_merge_commit_count: null
    }
  })
  assert.equal(protectedMainPush.status, "PASS", protectedMainPush.errors.join("\n"))

  for (const [name, pushGitBinding] of [
    [
      "wrong protected-main parent",
      {
        ...gitBinding,
        head: squashMergeSha,
        head_committed_at: "2026-09-08T14:20:00.000Z",
        head_parent_sha: "0".repeat(40),
        head_parent_shas: ["0".repeat(40)],
        post169_governance_seed_ancestor: false,
        post169_governance_amendment_commit_count: null,
        post169_governance_amendment_merge_commit_count: null
      }
    ...
    [
      "wrong protected-main tree",
      {
        ...gitBinding,
        head: squashMergeSha,
        head_committed_at: "2026-09-08T14:20:00.000Z",
        head_tree_sha: "0".repeat(40),
        head_parent_sha: post169GovernanceAuthorizationBaseSha,
        head_parent_shas: [post169GovernanceAuthorizationBaseSha],
        post169_governance_seed_ancestor: false,
        post169_governance_amendment_commit_count: null,
    ...
    const rejectedPush = runPost169GovernanceFixture(fixture, {
      currentHead: squashMergeSha,
      evaluatedHeadCommittedAt: "2026-09-08T14:20:00.000Z",
      githubActionsContext: pushContext,
      post169GovernanceAuthorizationReadback: makePost169GovernanceAuthorizationReadback({
      ...
      gitBinding: pushGitBinding
    })
    assert.equal(rejectedPush.status, "FAIL", name)
  }

  const expectedFinalHead = {
    branch: post169GovernanceBranch,
    sha: post169GovernancePriorFinalHeadSha,
    tree_sha: post169GovernancePriorFinalTreeSha,
    parent_sha: post169GovernanceSeedHeadSha,
  ...
  for (const [name, readbackOptions] of [
    ["missing", { finalSealReadbackOverrides: { status: "UNAVAILABLE" } }],
    ["non-OWNER", { finalSealReadbackOverrides: { user_login: "attacker" } }],
    ["edited", { finalSealReadbackOverrides: { updated_at: "2026-09-08T12:46:00Z" } }],
    [
      "stale head",
      {
        finalSealAuthorizationOverrides: {
          final_head: { ...expectedFinalHead, sha: "d".repeat(40) }
        }
      }
    ],
    [
      ...
      {
        finalSealAuthorizationOverrides: {
          final_head: { ...expectedFinalHead, tree_sha: "e".repeat(40) }
        }
      }
    ],
    [
      ...
      {
        finalSealAuthorizationOverrides: {
          final_head: { ...expectedFinalHead, parent_sha: "f".repeat(40) }
        }
      }
    ],
    [
      ...
      {
        finalSealAuthorizationOverrides: {
          authorized_paths: [...post169GovernanceAuthorizedPaths, "docs/research/unrelated.md"]
        }
      }
    ],
    [
      "wrong test blob",
      {
        finalSealAuthorizationOverrides: {
          content_blobs: {
            ...expectedContentBlobs,
            "scripts/test/validate-traceability.test.mjs": "1".repeat(40)
          }
        }
      }
    ],
    [
      "wrong validator blob",
      {
        finalSealAuthorizationOverrides: {
          content_blobs: {
            ...expectedContentBlobs,
            "scripts/validate-traceability.mjs": "2".repeat(40)
          }
        }
      }
    ]
    ...
    assert.equal(report.status, "FAIL", name)
    assert.match(report.errors.join("\n"), /remediation|final seal|bodies must match/u, name)
  }

  const expectedFinalHeadV4 = {
    branch: post169GovernanceBranch,
    sha: gitBinding.head,
    tree_sha: gitBinding.head_tree_sha,
    parent_sha: post169GovernanceSeedHeadSha,
  ...
  for (const [name, readbackOptions] of [
    ["missing v4", { finalSealV4ReadbackOverrides: { status: "UNAVAILABLE" } }],
    ["non-OWNER v4", { finalSealV4ReadbackOverrides: { user_login: "attacker" } }],
    ["edited v4", { finalSealV4ReadbackOverrides: { updated_at: "2026-09-08T14:16:00Z" } }],
    [
      "stale v4 head",
      {
        finalSealV4AuthorizationOverrides: {
          final_head: { ...expectedFinalHeadV4, sha: "4".repeat(40) }
        }
      }
    ],
    [
      ...
      {
        finalSealV4AuthorizationOverrides: {
          final_head: { ...expectedFinalHeadV4, tree_sha: "5".repeat(40) }
        }
      }
    ],
    [
      "wrong v4 test blob",
      {
        finalSealV4AuthorizationOverrides: {
          content_blobs: {
            ...expectedContentBlobsV4,
            "scripts/test/validate-traceability.test.mjs": "6".repeat(40)
          }
        }
      }
    ],
    [
      "wrong v4 validator blob",
      {
        finalSealV4AuthorizationOverrides: {
          content_blobs: {
            ...expectedContentBlobsV4,
            "scripts/validate-traceability.mjs": "7".repeat(40)
          }
        }
      }
    ]
    ...
    assert.equal(report.status, "FAIL", name)
    assert.match(report.errors.join("\n"), /squash-push|v4 final seal|bodies must match/u, name)
  }
})
...
test("completed T085 accepts the exact authenticated US6 maintenance bootstrap scope", () => {
  ...
  assert.equal(report.status, "PASS", report.errors.join("\n"))
  assert.equal(report.mode, "T085_COMPLETE_STEADY")
  assert.deepEqual(report.scope_validation.changed_paths, postT085MaintenanceAuthorizedPaths)
  assert.deepEqual(report.scope_validation.unauthorized_paths, [])
  assert.equal(
    report.source.post_t085_maintenance_authorization_readback.html_url,
    postT085MaintenanceAuthorizationRef
  )
  assert.equal(report.source.github_actions_context.pull_request_draft, true)
})

test("completed T085 maintenance scope is non-authoritative outside exact-head CI", () => {
  const fixture = makeCompletedFixture()
  fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
  const report = runPostT085MaintenanceFixture(fixture, {
...
for (const [name, contextFactory, expected] of [
  [
    "generic CI context",
    () =>
    ...
    /post-T085 maintenance authorization requires authenticated GitHub Actions context/
  ],
  [
  ...
  test(`completed T085 fails closed for ${name} in exact-head mode`, () => {
    const fixture = makeCompletedFixture()
    fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
    const githubActionsContext = contextFactory(fixture)
    if (githubActionsContext.status === "MATCHED_GITHUB_ACTIONS_METADATA") {
      writeExactHeadForActionsContext(fixture.root, githubActionsContext)
    }
    const report = runPostT085MaintenanceFixture(fixture, {
      requireExactHeadEvidence: true,
...
for (const [name, readback, expected] of [
  [
    "missing readback",
    null,
    /post-T085 maintenance requires a verified GitHub owner-authorization readback/
  ],
  [
    "unavailable readback",
    makePostT085MaintenanceAuthorizationReadback({
      readbackOverrides: { status: "UNAVAILABLE" }
    }),
    /post-T085 maintenance requires a verified GitHub owner-authorization readback/
  ],
  [
    "non-GitHub readback source",
    makePostT085MaintenanceAuthorizationReadback({
      readbackOverrides: { source: "fixture" }
    }),
    /post-T085 maintenance requires a verified GitHub owner-authorization readback/
  ],
  [
    ...
    makePostT085MaintenanceAuthorizationReadback({
      readbackOverrides: {
        html_url: postT085MaintenanceSupersededAuthorizationRefs[1],
        issue_url: "https://api.github.com/repos/bynanci/courtside-tw/issues/164"
      }
    }),
  ...
  [
    ...
    makePostT085MaintenanceAuthorizationReadback({
      readbackOverrides: {
        created_at: "2026-09-01T14:13:03Z",
        updated_at: "2026-09-01T14:13:03Z"
      }
    }),
    /post-T085 maintenance authorization timestamp must match the owner dispatch/
  ],
  [
    "non-owner readback",
    makePostT085MaintenanceAuthorizationReadback({
      readbackOverrides: { user_login: "attacker" }
    }),
  ...
  [
    ...
    makePostT085MaintenanceAuthorizationReadback({
      readbackOverrides: { author_association: "NONE" }
    }),
    /post-T085 maintenance authorization comment must be authored by the repository owner/
  ],
  [
    "edited readback",
    makePostT085MaintenanceAuthorizationReadback({
      readbackOverrides: { updated_at: "2026-09-01T14:13:03Z" }
    }),
    /post-T085 maintenance authorization comment must be immutable after creation/
  ],
  [
    "wrong base binding",
    makePostT085MaintenanceAuthorizationReadback({
      authorizationOverrides: {
        authorization_base: { branch: "main", sha: "0".repeat(40), protected: true }
      }
    }),
  ...
  [
    "wrong path binding",
    makePostT085MaintenanceAuthorizationReadback({
      authorizationOverrides: { authorized_paths: ["apps/web/tests/e2e/other.spec.ts"] }
    }),
    /post-T085 maintenance authorization body must match the exact owner dispatch/
  ],
  [
    "wrong branch binding",
    makePostT085MaintenanceAuthorizationReadback({
      authorizationOverrides: { branch: "fix/unrelated" }
    }),
    /post-T085 maintenance authorization body must match the exact owner dispatch/
  ],
  [
    "wrong pull-request binding",
    makePostT085MaintenanceAuthorizationReadback({
      authorizationOverrides: { pull_request: 164 }
    }),
    /post-T085 maintenance authorization body must match the exact owner dispatch/
  ],
  [
  ...
  [
    "missing supersession binding",
    makePostT085MaintenanceAuthorizationReadback({
      authorizationOverrides: { supersedes_authorization_refs: [] }
    }),
    /post-T085 maintenance authorization body must match the exact owner dispatch/
  ],
  [
  ...
  test(`completed T085 fails closed for ${name}`, () => {
    const fixture = makeCompletedFixture()
    fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
    const report = runPostT085MaintenanceFixture(fixture, {
      postT085MaintenanceAuthorizationReadback: readback
    })
...
test("completed T085 accepts an unfenced exact maintenance authorization body", () => {
  const fixture = makeCompletedFixture()
  fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
  const readback = makePostT085MaintenanceAuthorizationReadback()
  readback.body = readback.body.replace("```json\n", "").replace("\n```", "")
  const report = runPostT085MaintenanceFixture(fixture, {
    postT085MaintenanceAuthorizationReadback: readback
  })
...
test("completed T085 rejects a malformed maintenance authorization fence", () => {
  const fixture = makeCompletedFixture()
  fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
  const readback = makePostT085MaintenanceAuthorizationReadback()
  readback.body = readback.body.replace("```json", "```JSON")
  const report = runPostT085MaintenanceFixture(fixture, {
    postT085MaintenanceAuthorizationReadback: readback
  })

  assert.equal(report.status, "FAIL")
  assert.match(report.errors.join("\n"), /maintenance authorization JSON fence is invalid/)
})

test("completed T085 rejects duplicate keys in fenced maintenance authorization JSON", () => {
  const fixture = makeCompletedFixture()
  fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
  const duplicateKeyReadback = makePostT085MaintenanceAuthorizationReadback({
    readbackOverrides: {
      body: [
        "<!-- post-t085-maintenance:owner-authorization:start -->",
        "```json",
        `{"schema_version":"${postT085MaintenanceAuthorizationSchema}","schema_version":"forged"}`,
        "```",
        "<!-- post-t085-maintenance:owner-authorization:end -->"
      ].join("\n")
    }
  ...
  const report = runPostT085MaintenanceFixture(fixture, {
    postT085MaintenanceAuthorizationReadback: duplicateKeyReadback
  })
...
test("completed T085 rejects a partial US6 maintenance path grant", () => {
  const fixture = makeCompletedFixture()
  fixture.changedPaths = ["apps/web/tests/e2e/us6-offline-issue.spec.ts"]
  const report = runPostT085MaintenanceFixture(fixture)
...
test("completed T085 binds the maintenance support commit to the authorized E2E head", () => {
  const fixture = makeCompletedFixture()
  fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
  const report = runPostT085MaintenanceFixture(fixture, {
    gitBinding: {
      ...
      head_parent_sha: postT085MaintenanceAuthorizedHeadSha,
      head_parent_count: 1,
      post_t085_maintenance_authorized_head_ancestor: false,
      post_t085_maintenance_authorized_head_committed_at:
        postT085MaintenanceAuthorizedHeadCommittedAt,
      post_t085_maintenance_e2e_matches_authorized_head: true
    }
...
test("completed T085 binds the E2E bytes to the owner-signed candidate", () => {
  const fixture = makeCompletedFixture()
  fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
  const report = runPostT085MaintenanceFixture(fixture, {
    gitBinding: {
      ...
      post_t085_maintenance_authorized_head_committed_at:
        postT085MaintenanceAuthorizedHeadCommittedAt,
      post_t085_maintenance_e2e_matches_authorized_head: false
    }
  })
...
test("completed T085 requires OWNER authorization after the signed candidate commit", () => {
  const fixture = makeCompletedFixture()
  fixture.changedPaths = [...postT085MaintenanceAuthorizedPaths]
  const report = runPostT085MaintenanceFixture(fixture, {
    gitBinding: {
      ...
      post_t085_maintenance_authorized_head_ancestor: true,
      post_t085_maintenance_authorized_head_committed_at:
        postT085MaintenanceAuthorizationRecordedAt,
      post_t085_maintenance_e2e_matches_authorized_head: true
    }
  })
  ...
  assert.match(
    report.errors.join("\n"),
    /post-T085 maintenance authorization must postdate the owner-signed candidate/
  )
})

test("post-T085 maintenance authorization read-back runs only for its exact completed scope", () => {
  assert.equal(
    typeof traceabilityValidator.inspectPostT085MaintenanceAuthorizationForState,
    "function"
  )
  const fixture = makeCompletedFixture()
  let calls = 0
  const readback = traceabilityValidator.inspectPostT085MaintenanceAuthorizationForState(
    fixture.root,
    {
      changeBaseTasksText: fixture.changeBaseTasksText,
      changeBaseSha: postT085MaintenanceAuthorizationBaseSha,
      boundedScopeActive: false,
      changedPaths: [...postT085MaintenanceAuthorizedPaths],
  ...
  assert.equal(calls, 1)
  assert.equal(readback.html_url, postT085MaintenanceAuthorizationRef)
})

test("post-T085 maintenance read-back skips every non-exact scope", () => {
  const fixture = makeCompletedFixture()
  let calls = 0
  const inspect = () => {
    calls += 1
  ...
  for (const overrides of [
    { changedPaths: [postT085MaintenanceAuthorizedPaths[0]] },
    { changedPaths: [...postT085MaintenanceAuthorizedPaths, "docs/research/unrelated.md"] },
    { changeBaseSha: "0".repeat(40) },
    { boundedScopeActive: true }
  ]) {
    const readback = traceabilityValidator.inspectPostT085MaintenanceAuthorizationForState(
...
for (const supersededAuthorizationRef of postT085MaintenanceSupersededAuthorizationRefs) {
  test(`post-T085 maintenance read-back rejects superseded ${supersededAuthorizationRef}`, () => {
    const readback = traceabilityValidator.inspectPostT085MaintenanceAuthorization(
      supersededAuthorizationRef,
      { environment: {} }
    )
...
test("OWNER-comment read-back authenticates and paginates with the injected Actions credential", () => {
  const validator = fs.readFileSync(
    path.join(repositoryRoot, "scripts/validate-traceability.mjs"),
    "utf8"
  )

  assert.match(validator, /const token = process\.env\.GITHUB_TOKEN\?\.trim\(\)/)
  assert.match(validator, /headers\.Authorization = "Bearer " \+ token/)
  assert.match(validator, /childEnvironment\.GITHUB_TOKEN = environment\.GITHUB_TOKEN/)
  assert.match(validator, /response\.headers\.get\("link"\)/)
  assert.match(validator, /rel="next"/)
  assert.match(validator, /match_count: matches\.length/)
})
...
test("Git inspection binds the real maintenance DAG and signed E2E bytes", () => {
  ...
  execFileSync("git", ["clone", "--quiet", "--no-local", repositoryRoot, cloneRoot], {
    stdio: ["ignore", "ignore", "pipe"]
  })
  const repositoryHead = git(repositoryRoot, "rev-parse", "HEAD")
  git(cloneRoot, "checkout", "--detach", repositoryHead)
  git(cloneRoot, "config", "user.name", "Traceability Test")
  git(cloneRoot, "config", "user.email", "traceability@example.invalid")

  const e2ePath = path.join(cloneRoot, postT085MaintenanceAuthorizedPaths[0])
  fs.appendFileSync(e2ePath, "\n// unauthorized post-authorization drift\n")
  git(cloneRoot, "add", postT085MaintenanceAuthorizedPaths[0])
  git(cloneRoot, "commit", "-m", "mutate authorized E2E bytes")
  const mutatedInspection = traceabilityValidator.inspectGit(cloneRoot, { environment: {} })
  assert.equal(mutatedInspection.post_t085_maintenance_authorized_head_ancestor, true)
  assert.equal(mutatedInspection.post_t085_maintenance_e2e_matches_authorized_head, false)

  git(cloneRoot, "checkout", "--detach", postT085MaintenanceAuthorizationBaseSha)
  fs.writeFileSync(path.join(cloneRoot, "divergent-maintenance.txt"), "divergent\n")
  git(cloneRoot, "add", "divergent-maintenance.txt")
...
for (const [name, mutate] of [
  [
    "HOLD",
    (s) => {
      s.report.release_decision = "HOLD"
    }
  ],
  [
    "FAIL",
  ...
  [
    "UNKNOWN",
    (s) => {
      s.report.release_decision = "UNKNOWN"
    }
  ],
  [
    "missing report",
  ...
  [
    "untrusted producer",
    (s) => {
      s.producerTrusted = false
    }
  ],
  [
    "unavailable evidence",
    (s) => {
      s.requiredEvidenceFresh = false
    }
  ],
  [
    "stale head",
    (s) => {
      s.report.candidate_sha = "3".repeat(40)
    }
  ],
  [
    "stale protected base",
    (s) => {
      s.report.base.sha = "3".repeat(40)
    }
  ],
  [
    "stale merge base",
    (s) => {
      s.mergeBase = "3".repeat(40)
    }
  ],
  [
    "stale frozen hash",
    (s) => {
      s.frozenHash = "3".repeat(64)
    }
  ],
  [
    "deleted adjudication",
    (s) => {
      s.comments = []
    }
  ],
  [
    "spoofed owner",
    (s) => {
      s.comments[0].user.login = "attacker"
    }
  ],
  [
    "wrong association",
    (s) => {
      s.comments[0].author_association = "CONTRIBUTOR"
    }
  ],
  [
    "edited adjudication",
    (s) => {
      s.comments[0].updated_at = "2026-09-08T12:00:00Z"
    }
  ],
  [
    "new comment after decision",
    (s) => {
      s.comments[0].created_at = s.comments[0].updated_at = "2026-09-08T12:00:00Z"
    }
  ],
  [
    "incomplete blockers",
    (s) => {
      s.report.blockers.adjudicated = []
    }
  ],
  [
    "missing surface",
    (s) => {
      s.report.surfaces.required.pop()
    }
  ],
  [
    "failed stability",
    (s) => {
      s.report.stability.result = "FAIL"
    }
  ],
  [
    "expanded scope",
    (s) => {
      s.report.scope_boundaries.web3_activated = true
    }
  ],
  [
    "missing canonical digest",
    (s) => {
      delete s.report.adjudication.body_sha256
    }
  ],
  [
    "missing artifact freshness",
    (s) => {
      s.artifactCreatedAt = "2026-09-08T09:00:00Z"
    }
  ]
]) {
...
test("required T086 gate classifies only complete server-side non-T086 file lists", () => {
  ...
  assert.equal(
    classifyCandidate(
      171,
      [{ filename: "renamed.txt", previous_filename: "scripts/validate-beta-release.mjs" }],
      1
    ),
    "T086"
  )
...
test("required T086 gate ignores spoofed new comments but treats owner marker removal as invalidation", () => {
  const { canonicalComment } = requiredGateRuntime()
  const s = requiredGateFixture()
  assert.equal(
    canonicalComment([...s.comments, { ...s.comments[0], id: 999, user: { login: "attacker" } }])
      .id,
    987
  )
...
test("product remediation Git file closure rejects deleted required files and non-regular entries", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "product-remediation-files-"))
  try {
    for (const filePath of productRemediationPaths) {
      fs.mkdirSync(path.dirname(path.join(root, filePath)), { recursive: true })
      fs.writeFileSync(path.join(root, filePath), "fixture required file\n")
    }
    const baseline = initializeGitFixture(root)
    assert.equal(
      traceabilityValidator.inspectProductRemediationFiles(root, baseline),
      true,
      "all required regular files exist; optional files may remain absent"
    )
    for (const filePath of [
      "apps/api/src/test/java/tw/basketball/magazine/publication/PublicIssueApiIT.java",
      "apps/api/src/main/java/tw/basketball/magazine/security/RouteRateLimitFilter.java"
    ]) {
      git(root, "rm", "--", filePath)
      const deletedTree = git(root, "write-tree")
      assert.equal(
        traceabilityValidator.inspectProductRemediationFiles(root, deletedTree),
        false,
        `deleting required file must fail: ${filePath}`
      )
      git(root, "reset", "--hard", baseline)
    }
    const requiredPath = productRemediationPaths[0]
    fs.unlinkSync(path.join(root, requiredPath))
    fs.symlinkSync("untrusted-target", path.join(root, requiredPath))
    git(root, "add", "--", requiredPath)
    assert.equal(
      traceabilityValidator.inspectProductRemediationFiles(root, git(root, "write-tree")),
      false,
      "a required symlink must not count as a regular source file"
    )
    git(root, "reset", "--hard", baseline)
    const optionalPath = productRemediationOptionalPaths[0]
    fs.mkdirSync(path.dirname(path.join(root, optionalPath)), { recursive: true })
    fs.symlinkSync("untrusted-target", path.join(root, optionalPath))
    git(root, "add", "--", optionalPath)
    assert.equal(
      traceabilityValidator.inspectProductRemediationFiles(root, git(root, "write-tree")),
      false,
      "an optional symlink must also fail"
    )
  } finally {
...
for (const [name, mutate] of [
  [
    "missing accepted RED ancestry",
    (r) => {
      r.candidate.accepted_red_ancestor = false
    }
  ],
  [
    "changed accepted RED tree",
    (r) => {
      r.candidate.accepted_red_tree_sha = "b".repeat(40)
    }
  ],
  [
    "changed accepted RED parent",
    (r) => {
      r.candidate.accepted_red_parent_shas = ["b".repeat(40)]
    }
  ],
  [
    "extra pre-dispatch RED path",
    (r) => {
      r.candidate.accepted_red_changed_paths.push("scripts/validate-traceability.mjs")
    }
  ]
]) {
  test(`product remediation authorization rejects ${name}`, () => {
...
for (const supersededComment of [5594498950, 5594575189]) {
  test(`product remediation does not fall back to superseded canonical ${supersededComment}`, () => {
    const context = makeProductRemediationFixture()
    context.readback.authorization.html_url = `https://github.com/bynanci/courtside-tw/issues/121#issuecomment-${supersededComment}`
    const report = runProductRemediationFixture(context)
    assert.equal(report.status, "FAIL")
    assert.match(report.errors.join("\n"), /immutable issue 121 OWNER dispatch/u)
  })
...
// Studio uses a separate pinned authority; fixture trust never enters runCli/validateTraceability.
function makeStudioAuthorizationFixture({ push = false, draft = true } = {}) {
  const context = makeProductRemediationFixture({ push, draft })
  const config = {
    ref: "https://github.com/bynanci/courtside-tw/issues/121#issuecomment-6000000176",
  ...
  const dispatch = {
    schema_version: "courtside-studio-completion-owner-dispatch/v1",
    decision: "DISPATCH_ACCEPTED",
    repository: "bynanci/courtside-tw",
    pr: config.pr,
  ...
  const body =
    "<!-- studio-completion:owner-dispatch:v1:start -->\n```json\n" +
    JSON.stringify(dispatch) +
    "\n```\n<!-- studio-completion:owner-dispatch:v1:end -->"
  ...
  context.readback.protected_main.commit.sha = push ? context.head : config.base_sha
  ...
  const eventPath = path.join(context.fixture.root, "studio-event.json")
  fs.writeFileSync(
    eventPath,
    JSON.stringify(
      push
...
for (const state of ["draft", "ready", "squash-push"]) {
  test(`Studio completion separate gate accepts authenticated exact ${state}`, () => {
    ...
    assert.equal(
      traceabilityValidator
        .createStudioCompletionAuthorizationGate(config)
...
for (const [name, mutate] of studioNegativeCases) {
  test(`Studio completion separate gate rejects ${name}`, () => {
    const { config, options } = makeStudioAuthorizationFixture()
    mutate(options)
    const errors = []
    assert.equal(
...
for (const mutate of [
  ...
  test("Studio completion separate gate rejects a mismatched squash push", () => {
    const { config, options } = makeStudioAuthorizationFixture({ push: true })
    mutate(options)
    assert.equal(
      traceabilityValidator
        .createStudioCompletionAuthorizationGate(config)
...
test("Studio completion unbound descriptor cannot accept fixture authority or access GitHub", () => {
  const { config, options } = makeStudioAuthorizationFixture()
  const gate = traceabilityValidator.createStudioCompletionAuthorizationGate({
    ...traceabilityValidator.STUDIO_COMPLETION_AUTHORIZATION,
    ref: null,
    body_sha256: null,
    recorded_at: null,
    pr: null,
...
test("Studio completion Git inspector proves actual seed, history, frozen bytes and regular files", () => {
  const { config } = makeStudioAuthorizationFixture()
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "studio-authorization-git-"))
  const git = (...args) =>
    execFileSync("git", args, {
      cwd: root,
      encoding: "utf8",
      env: {
        ...process.env,
        GIT_AUTHOR_DATE: "2026-09-09T12:01:00+08:00",
        GIT_COMMITTER_DATE: "2026-09-09T12:01:00+08:00"
      },
      stdio: ["ignore", "pipe", "pipe"]
    }).trim()
  const write = (p, value) => {
  ...
  try {
    ...
    const gate = traceabilityValidator.createStudioCompletionAuthorizationGate(config)
    ...
    assert.deepEqual(candidate.seed_changed_paths, config.initial_seed.changed_paths)
...
test("Studio completion API read-back selects only the sealed PR/comment and sanitizes failure", () => {
  const { config, options } = makeStudioAuthorizationFixture()
  const gate = traceabilityValidator.createStudioCompletionAuthorizationGate(config)
  const calls = []
  const adapters = {
  ...
  assert.deepEqual(calls, [
    "comment",
    "https://api.github.com/repos/bynanci/courtside-tw/pulls/176",
...
test("Studio completion refuses malformed pinned JSON and mismatch between seal and dispatch", () => {
...
test("Studio completion singleton never accepts a caller-injected factory configuration", () => {
