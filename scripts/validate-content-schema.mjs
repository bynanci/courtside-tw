...
const validPath = path.join(root, "packages/content-schema/fixtures/valid/content-document-v1-all-blocks.json");
...
const invalidFiles = fs.readdirSync(invalidDir).filter((file) => file.endsWith(".json")).sort();
...
assert.equal(validate(duplicateFixture), true, "duplicate-ID fixture must isolate the semantic invariant");
const duplicateIds = duplicateFixture.blocks.map((block) => block.id);
assert.notEqual(new Set(duplicateIds).size, duplicateIds.length, "duplicate block IDs were not detected");
