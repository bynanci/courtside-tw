...
const schemaInvalidFiles = invalidFiles.filter(
  (file) => file !== duplicateFile,
);
assert.equal(
  schemaInvalidFiles.length,
  21,
  "unexpected schema-invalid fixture count",
);

for (const file of schemaInvalidFiles) {
  const fixture = JSON.parse(
    fs.readFileSync(path.join(invalidDir, file), "utf8"),
  );
  assert.equal(
    validate(fixture),
    false,
    file + " unexpectedly passed schema validation",
  );
}

const duplicateFixture = JSON.parse(
  fs.readFileSync(path.join(invalidDir, duplicateFile), "utf8"),
);
assert.equal(
