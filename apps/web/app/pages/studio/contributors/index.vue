...
import ContributorManager from "../../../features/studio/contributors/ContributorManager.vue"
import {
...
onMounted(async () => {
  try {
    ...
    if (!role.value) {
      error.value = "作者管理 需要 EDITOR role；URL 不能提升權限。"
    }
  } catch (cause) {
...
<template>
  ...
  <ContributorManager v-else-if="role === 'EDITOR'" :role="role" />
</template>
