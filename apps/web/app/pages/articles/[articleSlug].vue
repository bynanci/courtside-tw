...
const readingTimeMinutes = computed(() => {
  const characters = articleBlocks.value.reduce((total, block) => total + blockText(block).length, 0)
  return Math.max(1, Math.ceil(characters / 450))
})
const resumeStorageKey = computed(() =>
  article.value
    ? "courtside.reader.progress:" + article.value.slug + ":revision-" + article.value.revisionNumber
    : ""
)
          ...
          >
            ...
            <div v-else-if="block.type === 'gallery'" class="article-gallery">
              ...
              >
                ...
                <figcaption v-else-if="item.caption">{{ item.caption }}</figcaption>
              </figure>
            </div>
