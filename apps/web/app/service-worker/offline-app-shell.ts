...
export function shouldPrecacheAppShellPath(pathname: string): boolean {
  ...
  if (
    EXCLUDED_PRECACHE_ROUTE_PREFIXES.some((prefix) =>
...
function isAppShellRegistration(
  registration: AppShellRegistrationLike,
): boolean {
  ...
  return [
    ...
    (worker) =>
      worker && getPathname(worker.scriptURL) === APP_SHELL_WORKER_PATH,
  );
}
