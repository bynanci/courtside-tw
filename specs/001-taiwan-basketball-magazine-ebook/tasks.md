...
- [x] T056 [US3] Complete concurrency, expired-rights-at-execution, duplicate-worker, partial-external-failure and cache/search invalidation integration tests in `apps/api/src/test/java/tw/basketball/magazine/publication/PublicationReliabilityIT.java`; verify public origin denies withdrawn content before external purge succeeds. Completion proof: `PublicationReliabilityIT` is included in exact-head Java quality verification for PR #91; the suite covers duplicate and concurrent workers, rights expiry, durable surrogate-key invalidation retry, and withdrawn-origin denial before external purge.
