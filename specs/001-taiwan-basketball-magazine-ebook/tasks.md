...
- [x] T074 [US6] Implement temporary-download, checksum verification and atomic install/update/delete in `apps/web/app/features/offline/services/OfflineIssueManager.ts`; store manifest state in IndexedDB and never mark partial content complete. Completion proof: tests-only head `1ad03e6a5f05c5966c09c7ec234f5e7ccc2df731` produced the attributable CI #595 RED (`104` tests, `103` passed, `1` lifecycle failure); implementation head `c66aa78804f74c85d2b9a05c4cb65ce591f241a1` passed exact-head CI #596 and Security #597, including `104/104` web tests, the complete `54/54` browser regression and Lighthouse budgets. The final receipt-head verification is attached to PR #104; T075+ remains unchanged.
