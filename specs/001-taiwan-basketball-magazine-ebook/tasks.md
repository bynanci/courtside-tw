...
- [ ] T082 Create production deployment, expand/migrate/contract migration and rollback runbooks in `infra/deployment/`, `infra/docker/`, `docs/operations/deployment.md`, and `docs/operations/rollback.md`; test application rollback without destructive schema rollback. HOLD (2026-08-23): PR #127 merged runtime remediation to protected `main@ac699085cf699a9cf3eaf283911117bbdd570b80` from exact head `79dc274a5dde587e1e9c503c0701299fdfb5a9d9`, resolving the prior seven threads and passing exact-head CI #835, Security #837, 70 browser tests plus Lighthouse, and the isolated non-destructive rollback drill. A post-merge review then opened unresolved P1 thread `PRRT_kwDOTvfycs6bdbLN`: state schema v2 rejects existing v1 ledgers and provides no safe environment-bound migration or upgrade command, blocking even status and emergency rollback. Re-check only after tests-first v1→v2 ledger migration, preserved activation／rollback history, thread resolution, fresh exact-head CI／Security／browser／deployment gates and protected-main read-back; production activation, secrets, T083+, Web3 and participant research remain out of scope.
